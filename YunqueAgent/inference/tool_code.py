...
import json5
from base_tool import BaseTool, register_tool, extract_code, has_chinese_chars
...
@register_tool('CodeExecutor', allow_overwrite=True)
class CodeExecutor(BaseTool):
            ...
            for attempt in range(8):
                try:
                    ...
                    if code_result.run_result.execution_time >= timeout-1:
                        result.append(f"[CodeExecutor Error] TimeoutError: Execution timed out.")
                    result = '\n'.join(result)
                    print('SUCCESS RUNNING TOOL')
                    return result if result.strip() else 'Finished execution.'

                except Timeout as e:
                    last_error = f'[Code Executor Error] TimeoutError: Execution timed out on endpoint {endpoint}.'
                    ...
                    last_error = f'[Code Executor Error]: {str(e)} on endpoint {endpoint}'
                    print(f"Error on attempt {attempt + 1}: {last_error}")
                    if attempt == 4:  # Last attempt
                        return last_error
                    continue

            return last_error if last_error else '[Code Executor Error]: All attempts failed.'
    ...
    def call_specific_endpoint(self, params: Union[str, dict], endpoint: str, timeout: Optional[int] = 30, **kwargs) -> tuple:
        ...
        if not code.strip():
            return False, '[Code Executor Error]: Empty code.'
        ...
        try:
            ...
            return False, f'[Code Executor Error] TimeoutError: Execution timed out.', None
        except Exception as e:
            return False, f'[Code Executor Error]: {str(e)}', None
