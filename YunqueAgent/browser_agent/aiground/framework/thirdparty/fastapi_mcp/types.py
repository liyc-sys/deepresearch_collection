...
class OAuthMetadata(BaseType):
    ...
    def validate_endpoints_for_grant_types(self):
        if (
            "authorization_code" in self.grant_types_supported
            and not self.authorization_endpoint
        ):
            raise ValueError(
                "authorization_endpoint is required when authorization_code grant type is supported"
            )
        return self
...
class AuthConfig(BaseType):
    ...
    def validate_required_fields(self):
        if (
            self.custom_oauth_metadata is None
            and self.issuer is None
            and not self.dependencies
        ):
            raise ValueError(
                "at least one of 'issuer', 'custom_oauth_metadata' or 'dependencies' is required"
            )
        ...
        if self.setup_proxies:
            ...
            if self.setup_fake_dynamic_registration and not self.client_secret:
                raise ValueError(
                    "'client_secret' is required when 'setup_fake_dynamic_registration' is True"
                )
