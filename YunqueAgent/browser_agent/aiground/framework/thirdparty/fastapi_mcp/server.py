...
from typing_extensions import Annotated, Doc, TypeVar

import aiground.framework.thirdparty.mcp.types as types
from aiground.framework.thirdparty.mcp.server.lowlevel.server import Server
...
from .transport.http import FastApiHttpSessionManager
from .transport.sse import FastApiSseTransport
from .types import AuthConfig, HTTPRequestInfo
...
class FastApiMCP:
    ...
    def __init__(
        ...
        describe_all_responses: Annotated[
            bool,
            Doc(
                "Whether to include all possible response schemas in tool descriptions"
            ),
        ] = False,
        describe_full_response_schema: Annotated[
            bool,
            Doc(
                "Whether to include full json schema for responses in tool descriptions"
            ),
        ] = False,
        ...
        exclude_operations: Annotated[
            Optional[List[str]],
            Doc(
                "List of operation IDs to exclude from MCP tools. Cannot be used with include_operations."
            ),
        ] = None,
        include_tags: Annotated[
            Optional[List[str]],
            Doc(
                "List of tags to include as MCP tools. Cannot be used with exclude_tags."
            ),
        ] = None,
        exclude_tags: Annotated[
            Optional[List[str]],
            Doc(
                "List of tags to exclude from MCP tools. Cannot be used with include_tags."
            ),
        ] = None,
        ...
        session_resource_manager: Annotated[
            Optional[SessionResourceManager],
            Doc(
                """
                Session resource manager to use for managing session resources.
                """
            ),
        ] = None,
    ):
        ...
        if include_operations is not None and exclude_operations is not None:
            raise ValueError(
                "Cannot specify both include_operations and exclude_operations"
            )
        ...
        self._http_transport: FastApiHttpSessionManager | None = (
            None  # Store reference to HTTP transport for cleanup
        )
    ...
    def setup_server(self) -> None:
        ...
        mcp_server: Server = Server(self.name, self.description, lifespan=self.lifespan)
            ...
            try:
                ...
                if request_context and hasattr(request_context, "request"):
                    ...
                    if http_request and hasattr(http_request, "method"):
                        ...
                        logger.debug(
                            "Extracted HTTP request info from context: %s, %s, %s",
                            http_request_info.method,
                            http_request_info.path,
                            http_request_info.query_params,
                        )
    ...
    def _register_mcp_endpoints_sse(
        ...
        self._register_mcp_connection_endpoint_sse(
            router, transport, mount_path, dependencies
        )
        self._register_mcp_messages_endpoint_sse(
            router, transport, mount_path, dependencies
        )
    ...
    def _setup_auth_2025_03_26(self):
        ...
        if self._auth_config:
            if self._auth_config.custom_oauth_metadata:
                ...
                if not metadata_url:
                    metadata_url = (
                        f"{self._auth_config.issuer}{self._auth_config.metadata_path}"
                    )
    ...
    def mount_http(
        ...
        assert isinstance(
            router, (FastAPI, APIRouter)
        ), f"Invalid router type: {type(router)}"
        ...
        self._register_mcp_endpoints_http(
            router, http_transport, mount_path, dependencies
        )
        self._setup_auth()
    ...
    def mount_sse(
        ...
        base_path = (
            mount_path if isinstance(router, FastAPI) else router.prefix + mount_path
        )
        messages_path = f"{base_path}/messages/"

        sse_transport = FastApiSseTransport(
            messages_path, session_resource_manager=self.session_resource_manager
        )
        dependencies = self._auth_config.dependencies if self._auth_config else None

        self._register_mcp_endpoints_sse(
            router, sse_transport, mount_path, dependencies
        )
        self._setup_auth()
    ...
    async def _execute_api_tool(
        ...
        operation_map: Annotated[
            Dict[str, Dict[str, Any]],
            Doc("A mapping from tool names to operation details"),
        ],
        ...
        arguments = (
            arguments.copy() if arguments else {}
        )  # Deep copy arguments to avoid mutating the original
        ...
        if http_request_info and http_request_info.query_params:
            session_id = http_request_info.query_params.get("session_id", "")
            if session_id:
        ...
        try:
            ...
            logger.exception("Error calling %s, err: %s", tool_name, e)
            raise e
    ...
    def _filter_tools(
        self, tools: List[types.Tool], openapi_schema: Dict[str, Any]
    ) -> List[types.Tool]:
        """
        Filter tools based on operation IDs and tags.
