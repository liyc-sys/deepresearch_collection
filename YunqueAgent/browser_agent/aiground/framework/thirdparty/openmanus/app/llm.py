...
class LLM:
    ...
    def format_messages(
        ...
        for message in messages:
            ...
            if isinstance(message, dict):
                ...
                if supports_images and message.get("base64_image"):
                    ...
                    image_message_indices.append(len(formatted_messages))
        ...
        if len(image_message_indices) > 1:
            # If there are multiple image messages,
            # remove the content of all but the last image message
            for index in image_message_indices[:-1]:
                content = formatted_messages[index]["content"]
                new_content = []
                for item in content:
                    if item["type"] != "image_url":
                        new_content.append(item)
                formatted_messages[index]["content"] = new_content
