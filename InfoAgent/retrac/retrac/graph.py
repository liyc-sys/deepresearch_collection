...
def _save_report(content: str, question: str) -> None:
    """Save report to outputs/ directory with timestamp filename."""
    out_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "outputs")
    os.makedirs(out_dir, exist_ok=True)
    # Build filename from timestamp + sanitized question snippet
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    slug = re.sub(r'[^\w\u4e00-\u9fff]+', '_', question[:40]).strip('_')
    path = os.path.join(out_dir, f"{ts}_{slug}.md")
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"\n[InfoAgent] Report saved to: {path}")
...
def build_graph(cfg: dict) -> StateGraph:
    ...
    async def end_cycle(state: WebCycleResearchState) -> WebCycleResearchState:
        ...
        summary_msg = summary.get('messages', [])[-1] if summary.get('messages') else None
        cycle_histories = state["cycle_histories"] + [[summary_msg]]
    ...
    async def start_cycle(state: WebCycleResearchState) -> WebCycleResearchState:
        ...
        for cycle_history in state["cycle_histories"]:
            for message in cycle_history:
                if message is None:
                    continue
                text = message.text if isinstance(message, BaseMessage) else str(message)
                # Remove reasoning part if exists
                if "</think>" in text:
                    text = text.split("</think>")[-1]
                contents_without_think.append(text)
    ...
    async def final(state: WebCycleResearchState) -> Dict[str, Any]:
        # If report_synthesis_prompt is configured, synthesize a full report
        # from all cycle summaries; otherwise fall back to original QA behavior.
        if domain_cfg.report_synthesis_prompt:
            # Collect all cycle summaries
            all_summaries_parts = []
            for i, cycle_history in enumerate(state.get("cycle_histories", []), 1):
                for message in cycle_history:
                    text = message.text if isinstance(message, BaseMessage) else str(message)
                    # Strip reasoning tags if present
                    if "</think>" in text:
                        text = text.split("</think>")[-1]
                    all_summaries_parts.append(f"### Research Cycle {i}\n{text}")

            all_summaries = "\n\n---\n\n".join(all_summaries_parts)

            synthesis_prompt = domain_cfg.report_synthesis_prompt.format(
                input=state["question"],
                all_summaries=all_summaries,
            )
            result = await llm_node(
                {"messages": [
                    SystemMessage(content="You are an expert research report writer. Synthesize research findings into a comprehensive, well-structured report."),
                    HumanMessage(content=synthesis_prompt),
                ]}
            )
            report_text = result.get("messages", [None])[-1]
            if report_text and isinstance(report_text, BaseMessage):
                output = report_text.text
                if "</think>" in output:
                    output = output.split("</think>")[-1]
                _save_report(output, state.get("question", ""))
