...
async def get_or_create_session(session_id: str) -> UserSession:
    ...
    # Try Docker sandbox first; fall back to LocalBackend if Docker unavailable
    sandbox = None
    if session_manager is not None:
        try:
            sandbox = await session_manager.get_or_create(session_id)
        except Exception as e:
            logger.warning(f"Docker sandbox unavailable ({e}), falling back to LocalBackend")

    if sandbox is None:
        workspace_dir = WORKSPACES_DIR / session_id
        workspace_dir.mkdir(parents=True, exist_ok=True)
        sandbox = LocalBackend(root_dir=str(workspace_dir))

    # Seed workspace with context files (ignore errors for LocalBackend path issues)
    try:
        ...
        logger.debug(f"Could not seed workspace files: {e}")
