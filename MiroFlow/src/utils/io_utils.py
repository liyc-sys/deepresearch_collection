...
class OutputFormatter:
    ...
    def format_final_summary_and_log(self, final_answer_text, client=None, output_mode="qa"):
        """Format final summary information, including answer and token statistics"""
        if output_mode == "report":
            # Report mode: return the full report text as-is
            summary_lines = []
            summary_lines.append("\n" + "=" * 30 + " Research Report " + "=" * 30)
            summary_lines.append(final_answer_text)
            return "\n".join(summary_lines), final_answer_text

        # QA mode: original behavior
        summary_lines = []
        summary_lines.append("\n" + "=" * 30 + " Final Answer " + "=" * 30)
        summary_lines.append(final_answer_text)
    ...
    def save_report_to_file(self, report_text, task_id, output_dir="logs"):
        """Save the research report as a standalone .md file"""
        os.makedirs(output_dir, exist_ok=True)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        # Sanitize task_id for filename
        safe_task_id = "".join(c if c.isalnum() or c in "-_" else "_" for c in task_id)
        filename = f"{timestamp}_{safe_task_id}_report.md"
