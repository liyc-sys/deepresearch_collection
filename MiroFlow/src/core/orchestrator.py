...
class Orchestrator:
    ...
    async def run_main_agent(
        ...
        output_mode = self.cfg.main_agent.output_process.get("output_mode", "qa")

        if output_mode == "report":
            task_guidence = """

Your task is to conduct comprehensive, systematic research on the given topic by actively collecting detailed information from the web. Your goal is to gather thorough, well-organized material that will be synthesized into a complete research report at the end.

Research strategy:
- Collect comprehensive information from multiple reliable sources to cover all aspects of the research topic.
- Prioritize breadth and depth: explore the topic from multiple angles, sub-topics, and perspectives.
- Gather specific facts, data points, statistics, expert opinions, and concrete examples.
- Document the source of each piece of information for proper citation in the final report.
- When encountering conflicting information, collect evidence from both sides rather than picking one.
- Look for recent and authoritative sources: academic papers, official reports, reputable news outlets, and expert analyses.
- If the research topic specifies a particular structure or set of sub-questions, ensure each one is thoroughly addressed.

Your objective is to build a comprehensive knowledge base on the topic that will enable the generation of a thorough, evidence-based research report.
"""
        else:
            task_guidence = """
...
"""
        ...
        if final_answer_text:
            ...
            if output_mode == "report":
                # Report mode: skip answer extraction, use summary directly as the report
                logger.info("Report mode: skipping final answer extraction")
            elif self.cfg.main_agent.output_process.final_answer_extraction:
                # QA mode: Use LLM to extract final answer
                extracted_answer = ""
        ...
        final_summary, final_boxed_answer = (
            self.output_formatter.format_final_summary_and_log(
                final_answer_text, self.llm_client, output_mode=output_mode
            )
        )

        # Save report as .md file in report mode
        if output_mode == "report" and final_answer_text and final_answer_text != "No final answer generated.":
            report_path = self.output_formatter.save_report_to_file(
                final_answer_text, task_id, self.cfg.output_dir
            )
            self.task_log.log_step(
                "report_saved", f"Research report saved to: {report_path}"
            )
        ...
        elif "browsecomp-zh" in self.cfg.benchmark.name:
            return final_summary, final_summary
        else:
